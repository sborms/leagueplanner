...
from leagueplanner.constants import MAX_ALLOWED_REST_DAYS, OUTPUT_COLS
...
st.set_page_config(page_title="League Planner", page_icon="⚽📅", layout="wide")

st.markdown("## 📅 League Planner")
...
with main_col1:
    ...
    st.info(
        "One league per sheet, use **NIET** for unavailability ([example](https://github.com/sborms/leagueplanner/blob/main/example/input.xlsx))",
        icon="ℹ️",
    )
...
with main_col2:
    ...
    st.info(
        "See `help(PlannerParams)` for full parametrization",
        icon="ℹ️",
    )
...
with output_col2:
    if len(output_sch) > 0:
        ...
        st.markdown(
            ...
            "The algorithm itself **does not penalize** excessive rest days for those teams, hence the latter column is most relevant. As the table displays the actual rest days and not the adjusted "
